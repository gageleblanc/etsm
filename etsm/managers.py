...
class SourcesManager:
    ...
    def __init__(self, debug: bool = False, sources_url: str = None):
        ...
        if self.index is None:
            self.logger.error("Failed to get sources index")
    ...
    def download_sources(self, all_versions: bool = False):
...
class ServerManager:
    ...
    def list_enabled_maps(self):
        pk3s = [x.name[:-len(".pk3")] for x in (self.source_path / "maps").iterdir() if x.is_file() and x.name.endswith(".pk3")]
        # Remove etmain paks
        pk3s.remove("pak0.pk3")
        pk3s.remove("pak1.pk3")
        pk3s.remove("pak2.pk3")
        return pk3s
